...
sap.ui.define([
    ...
    opaTest("Service tile grid renders with items on the Find Help tab", function (Given, When, Then) {
        ...
        Then.waitFor({
            ...
            matchers: function (oItem) {
                return oItem.$().find(".fiSvcName").length > 0;
            },
    ...
    opaTest("No 'Popular' badge appears on Gardening tile", function (Given, When, Then) {
        ...
        // Gardening tile should not carry the Popular badge
        Then.waitFor({
            controlType: "sap.m.CustomListItem",
            viewName: "helphub.view.Dashboard",
            matchers: function (oItem) {
                var oIcon = oItem.$().find(".fiSvcName");
                return oIcon.length && oIcon.text() === "Gardening";
            },
            success: function (aItems) {
                var oTile = aItems[0];
                var aStatuses = oTile.findAggregatedObjects(true, function (o) {
                    return o.isA("sap.m.ObjectStatus") && o.getText() === "Popular";
                });
                Opa5.assert.strictEqual(aStatuses.length, 0,
                    "Gardening tile does not carry a Popular badge");
            },
    ...
    aCategorySpotChecks.forEach(function (sCat) {
        opaTest("Service tile for '" + sCat + "' is present", function (Given, When, Then) {
            ...
            Then.waitFor({
                ...
                matchers: function (oItem) {
                    var oIcon = oItem.$().find(".fiSvcName");
                    return oIcon.length && oIcon.text() === sCat;
                },
                success: function (aItems) {
                    Opa5.assert.ok(aItems.length > 0, "Tile for '" + sCat + "' found");
                },
                errorMessage: "Service tile for '" + sCat + "' not found"
            });
